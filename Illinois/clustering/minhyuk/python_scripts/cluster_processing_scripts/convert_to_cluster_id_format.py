...
def parse_parsing_clusters(clustering_output, output_prefix):
    '''This function takes in the raw parsing clusters format
    which is "node_id,cluster_id"
    and writes a cluster format where each line is "<cluster number>SPACE<node id>"
    '''
    with open(output_prefix, "w") as fw:
        with open(clustering_output, "r") as f:
...
@click.option("--cluster-method", required=True, type=click.Choice(["mcl", "leiden", "ikc", "graclus", "parsing_clusters"]), help="Clustering method used")
...
def convert_to_cluster_id_format(clustering_output, leiden_mapping, cluster_method, output_prefix):
    ...
    elif(cluster_method == "parsing_clusters"):
        parse_parsing_clusters(clustering_output, output_prefix)
