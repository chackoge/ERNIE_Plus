...
def find_best_cluster(current_id, graph, id_to_cluster_dict, cluster_to_id_dict, core_nodes, min_degree, cluster_criterion):
    ...
    for cluster_id in cluster_count_dict:
        int_cluster_member_arr = [int(node_id) for node_id in cluster_to_id_dict[cluster_id]]
        current_proportion = cluster_count_dict[cluster_id] / len(set(int_cluster_member_arr).intersection(set(core_nodes)))
