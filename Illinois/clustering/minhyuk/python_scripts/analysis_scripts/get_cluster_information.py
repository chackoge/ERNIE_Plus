...
def evaluate_clustering(cluster_to_id_dict, id_to_cluster_dict, graph, figure_prefix, output_prefix):
    ...
    with open(f"{output_prefix}_clustering.info", "w") as f:
...
def track_nodes(nodes_of_interest, cluster_to_id_dict, id_to_cluster_dict, output_prefix):
   ...
   for node_id in nodes_of_interest:
       ...
       if(str(node_id) in id_to_cluster_dict):
           ...
           marker_node_dict[node_id] = current_cluster_id
           if(current_cluster_id not in cluster_id_to_marker_dict):
               cluster_id_to_marker_dict[current_cluster_id] = 0
           cluster_id_to_marker_dict[current_cluster_id] += 1

   with open(f"{output_prefix}/nodes_of_interest.info", "w") as f:
       f.write(f"node,cluster_id,cluster_size,num_markers_in_cluster\n")
       for node_id,cluster_id in marker_node_dict.items():
           num_markers_in_cluster = cluster_id_to_marker_dict[cluster_id]
           f.write(f"{node_id},{cluster_id},{len(cluster_to_id_dict[int(cluster_id)])},{num_markers_in_cluster}\n")
...
def evaluate_individual_clusters(cursor, table_name, cluster_to_id_dict, id_to_cluster_dict, graph, output_prefix, output_to_file=True):
    non_singleton_cluster_arr = []
    singleton_count = 0
    coverage = 0
    for cluster_id,cluster_member_arr in cluster_to_id_dict.items():
        current_length = len(cluster_member_arr)
        if(current_length == 1):
           singleton_count += 1
            ...
            non_singleton_cluster_arr.append(current_length)
            coverage += len(cluster_member_arr)
        ...
        for cluster_id,cluster_member_arr in cluster_to_id_dict.items():
            if(len(cluster_member_arr) > 1):
                ...
                for cluster_member in int_cluster_member_arr:
                    ...
                    volume += graph.degree(cluster_member)
                min_k_arr.append(current_min_k)
                l_s /= 2
                assert current_min_k != 0
    ...
    if(output_to_file):
        with open(f"{output_prefix}_k_arr.info", "w") as f:
            f.write(f"// this file contains the distribution information about minimum k in each cluster\n")
            f.write(f"min k: {np.min(min_k_arr)}\n")
            f.write(f"max k: {np.max(min_k_arr)}\n")
            f.write(f"mean k: {np.mean(min_k_arr)}\n")
            f.write(f"median k: {np.median(min_k_arr)}\n")
            f.write(f"10th percentile k: {np.percentile(min_k_arr, 10)}\n")
            f.write(f"25th percentile k: {np.percentile(min_k_arr, 25)}\n")
            f.write(f"75th percentile k: {np.percentile(min_k_arr, 75)}\n")
            f.write(f"90th percentile k: {np.percentile(min_k_arr, 90)}\n")
            f.write(f"91th percentile k: {np.percentile(min_k_arr, 91)}\n")
            f.write(f"92th percentile k: {np.percentile(min_k_arr, 92)}\n")
            f.write(f"93th percentile k: {np.percentile(min_k_arr, 93)}\n")
            f.write(f"94th percentile k: {np.percentile(min_k_arr, 94)}\n")
            f.write(f"95th percentile k: {np.percentile(min_k_arr, 95)}\n")
            f.write(f"96th percentile k: {np.percentile(min_k_arr, 96)}\n")
            f.write(f"97th percentile k: {np.percentile(min_k_arr, 97)}\n")
            f.write(f"98th percentile k: {np.percentile(min_k_arr, 98)}\n")
            f.write(f"99th percentile k: {np.percentile(min_k_arr, 99)}\n")
            f.write(f"99.9th percentile k: {np.percentile(min_k_arr, 99.9)}\n")
            f.write(f"99.99th percentile k: {np.percentile(min_k_arr, 99.99)}\n")
            f.write(f"99.999th percentile k: {np.percentile(min_k_arr, 99.999)}\n")
            f.write(f"99.9999th percentile k: {np.percentile(min_k_arr, 99.9999)}\n")

        with open(f"{output_prefix}_modularity.info", "w") as f:
            ...
            f.write(f"75th percentile modularity:  {np.percentile(modularity_arr, 75)}\n")
            f.write(f"90th percentile modularity:  {np.percentile(modularity_arr, 90)}\n")
        ...
        with open(f"{output_prefix}_conductance.info", "w") as f:
            ...
            f.write(f"75th percentile conductance:  {np.percentile(conductance_arr, 75)}\n")
            f.write(f"90th percentile conductance:  {np.percentile(conductance_arr, 90)}\n")
        ...
        with open(f"{output_prefix}_latex.format", "w") as f:
            num_non_sing = len(non_singleton_cluster_arr)
            num_sing = int(singleton_count)
            coverage = int(coverage)
            med_size = int(np.median(non_singleton_cluster_arr))
            max_size = int(np.max(non_singleton_cluster_arr))
            min_mcd = int(np.min(min_k_arr))
            med_mcd = int(np.median(min_k_arr))
            max_mcd = int(np.max(min_k_arr))
            precision = 2
            min_mod = np.format_float_scientific(np.min(modularity_arr), precision=precision)
            med_mod = np.format_float_scientific(np.median(modularity_arr), precision=precision)
            max_mod = np.format_float_scientific(np.max(modularity_arr), precision=precision)
            min_cond = np.format_float_positional(np.min(conductance_arr), precision=precision)
            med_cond = np.format_float_positional(np.median(conductance_arr), precision=precision)
            max_cond = np.format_float_positional(np.max(conductance_arr), precision=precision)
            f.write(f"{num_non_sing}/{num_sing} & {coverage} & {med_size}/{max_size} & {min_mcd}/{med_mcd}/{max_mcd} & {min_mod}/{med_mod}/{max_mod} & {min_cond}/{med_cond}/{max_cond} \\\\ \n")
...
@click.option("--nodes-file", required=False, type=click.Path(exists=True), help="Nodes file")
