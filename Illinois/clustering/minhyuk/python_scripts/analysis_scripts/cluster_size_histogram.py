...
def cluster_statistics(input_clusters, output_prefix):
    ...
    "<cluster numbr>SPACE<node id>"and writes a histogram of cluster sizes.
    '''
    cluster_dict = {}
    with open(input_clusters, "r") as f:
        for line in f:
            line_arr = line.split()
            current_cluster_membership = line_arr[0]
            current_node_num = line_arr[1]
    ...
    bins = np.arange(0, max(cluster_size_data) + 1, 1)
