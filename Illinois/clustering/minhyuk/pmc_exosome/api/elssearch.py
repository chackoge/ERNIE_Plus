...
class ElsSearch():
    ...
    def __init__(self, id, query, index):
    ...
    def get_datapath(self, client):
        return client.output_dir / str("elssearch-" + self.id +'.json')
    ...
    def execute(self, els_client = None, get_all = False):
        ...
        if (self._results):
            dataPath = self.get_datapath(els_client)
            with dataPath.open(mode='w') as dump_file:
                json.dump(self._results, dump_file)
                dump_file.close()
            els_client.logger.info('Wrote ' + self.uri + ' to file')
            return True
        else:
            els_client.logger.warning('No data to write for ' + self.uri)
            return False
        self.results_df = utils.recast_df(pd.DataFrame(self._results))
        return True
