...
/usr/bin/time -v java -cp /srv/local/shared/external/leiden/networkanalysis-1.1.0 nl.cwts.networkanalysis.run.RunNetworkClustering -r ${RESOLUTION} -o ${OUTPUT_FILE} ${INPUT_NETWORK} 2> ${OUTPUT_PREFIX}/leiden_${SIZE}.err 1> ${OUTPUT_PREFIX}/leiden_${SIZE}.out
