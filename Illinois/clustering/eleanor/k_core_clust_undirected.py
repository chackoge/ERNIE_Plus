...
def main(args):
    ...
    while components:
        ...
        while sub_components:
            ...
            # recompute the components and update the componet lists or dictionaries
...
def k_core(max_degree, node_dict):
    ...
    for label, node in node_dict.items():
        if node.get_degree() <= max_degree:
            heapq.heappush(queue, [node.get_degree(), counter, node])
            counter += 1
            queued.add(node)
    degree = 0
    while queue:
        ...
        if degree <= max_degree:
            if node.get_label() in node_dict:
                ...
                for neighbor in node.get_neighbors():
                    ...
                    if neighbor.get_degree() <= max_degree:
                        if neighbor not in queued:
                            heapq.heappush(queue, [neighbor.get_degree(), counter, neighbor])
                            counter += 1
                            queued.add(neighbor)
