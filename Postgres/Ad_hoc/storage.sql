...
SELECT pt.spcname AS tablespace, pg_size_pretty(coalesce(pg_tablespace_size(pt.spcname), 0)) AS used_space,
  CASE spcname
    ...
    ELSE pg_tablespace_location(pt.oid) END AS location, pt.oid AS tablespace_oid
...
SELECT pc.relname, pg_size_pretty(pg_total_relation_size(pc.oid)),
  ...
  CASE
    WHEN pc.reltablespace = 0 -- DB default
      THEN db_pt.spcname
    ELSE coalesce(obj_pt.spcname, 'INVALID') END AS tablespace, pc.reltablespace AS tablespace_oid
FROM pg_class pc --
       LEFT JOIN pg_tablespace obj_pt ON obj_pt.oid = pc.reltablespace
       JOIN pg_database pd ON pd.datname = current_catalog
       JOIN pg_tablespace db_pt ON db_pt.oid = pd.dattablespace
...
SELECT pt.spcname,
  format('CREATE TABLESPACE %I LOCATION %L;', pt.spcname, CASE spcname
                                                            WHEN 'pg_default' THEN ps.setting || '/base'
                                                            WHEN 'pg_global' THEN ps.setting || '/global'
                                                            ELSE pg_tablespace_location(pt.oid) END) AS ddl,
