...
if [[ $1 == "-h" || $# -lt 2 ]]; then
  cat <<'HEREDOC'
NAME
  neo4j-switch-db.sh -- switches Neo4j 4 default DB and restarts Neo4j

SYNOPSIS
  neo4j-switch-db.sh neo4j_db current_user_password
...
echo "$2" | sudo --stdin -u neo4j bash -c "set -e
