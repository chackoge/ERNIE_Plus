...
CREATE TABLE open_citation_pubs (
  omid CHAR(19) NOT NULL,
  uri CHAR(39) NOT NULL,
  iid INT NOT NULL GENERATED BY DEFAULT AS IDENTITY (MINVALUE 0),
  title VARCHAR(1000),
  author_uris VARCHAR(1000) NOT NULL,
  issue VARCHAR(100),
  volume VARCHAR(100),
  venue_uri CHAR(39),
  pages VARCHAR(100),
...
COMMENT ON COLUMN open_citation_pubs.uri IS 'The publication URI';
...
COMMENT ON COLUMN open_citation_pubs.author_uris IS 'The author URIs, "; "-separated';
...
SELECT 'foo' AS id, title, author_uris AS author, issue, volume, venue_uri AS venue, pages AS page, 'bar' AS pub_date,
  type, publisher_uri AS publisher, editor_uris AS editor
...
\include_relative trg_transform_and_load_open_citation_pub.sql
