...
CREATE TABLE IF NOT EXISTS open_citation_pubs (
  ...
  uri VARCHAR(400) NOT NULL, -- TBD length ?
  iid INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY (MINVALUE 0),
  title VARCHAR(40000),
  authors VARCHAR(500000),
  issue VARCHAR(50), -- TBD length ?
  volume VARCHAR(50), -- TBD length ?
  venue VARCHAR(8000),
  pages VARCHAR(50), -- TBD length ?
  pub_year SMALLINT,
  pub_month SMALLINT,
  pub_date DATE,
  type VARCHAR(100), -- TBD replace by ENUM
  publisher VARCHAR(400), -- TBD length ?
...
COMMENT ON COLUMN open_citation_pubs.type IS --
  'The publication type: one of "journal article", "book chapter", "proceedings", "proceedings article"';
--@formatter:off
